...
/**
 * @Classname ThreeCapDumpWater
 * @Description 三个杯子平分8升水
 * @Date 2019/11/16 16:42
 * @Created by XL
 */
public class ThreeCapDumpWater {
    public static int CAP_NUM = 3;
    ...
    public static int[] CAP_CAPACITY = new int[]{8, 5, 3};
        ...
        public boolean equals(Object obj) {
            if (this == obj) {
                ...
                if (obj instanceof State) {
                    int[] states = ((State) obj).state;
                    return Arrays.equals(states,this.state) ? true:false;
                } else {
                    return false;
                }
            }
    ...
    private static void recursionSearchState(LinkedList<State> stateList) {
        ...
        if (currentState.equals(STOPED_STATE)) {
            ...
            IntStream.range(0,CAP_NUM).forEach(i-> IntStream.range(0,CAP_NUM).filter(j -> i!=j ).forEach(j->tryDumpWater(stateList,currentState,i,j)));
        }
    }
    ...
    private static boolean dumpWater(State currentState, State nextState, int from, int to) {
        int dumpCapacity = CAP_CAPACITY[to] - currentState.state[to];
        ...
        if (currentState.state[from] <= dumpCapacity) {
            ...
            nextState.state[to] = CAP_CAPACITY[to];
        }
        if (dumpCapacity > 0) {
    ...
    private static boolean canDumpWater(int i, int j, State currentState) {
        return i != j && currentState.state[i] != 0 && CAP_CAPACITY[j] - currentState.state[j] > 0 ? true : false;
    }
